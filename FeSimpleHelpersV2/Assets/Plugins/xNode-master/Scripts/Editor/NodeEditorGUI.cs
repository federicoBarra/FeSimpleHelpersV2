        ...
        public void DrawNoodle(Gradient gradient, NoodlePath path, NoodleStroke stroke, float thickness, List<Vector2> gridPoints) {
            ...
            switch (path) {
                ...
                case NoodlePath.ShaderLab:
                    ...
                    break;
                case NoodlePath.OrthogonalAvoid:
	                const float horizontalPadding = 40f;

	                for (int i = 0; i < length - 1; i++)
	                {
		                Vector2 from = gridPoints[i];
		                Vector2 to = gridPoints[i + 1];

		                float dir = Mathf.Sign(to.x - from.x);
		                if (dir == 0f) dir = 1f;

		                float pad = horizontalPadding / zoom;

		                // Orthogonal points
		                Vector2 p1 = from;
		                Vector2 p2 = from + Vector2.right * dir * pad;
		                Vector2 p3 = new Vector2(p2.x, to.y);
		                Vector2 p4 = to;

		                // First segment
		                if (i == length - 2) Handles.color = gradient.Evaluate(0f);
		                DrawAAPolyLineNonAlloc(thickness, p1, p2);

		                // Second segment
		                if (i == length - 2) Handles.color = gradient.Evaluate(0.5f);
		                DrawAAPolyLineNonAlloc(thickness, p2, p3);

		                // Third segment
		                if (i == length - 2) Handles.color = gradient.Evaluate(1f);
		                DrawAAPolyLineNonAlloc(thickness, p3, p4);
	                }
					break;
