...
namespace FeSimpleHelpers
{
	public static class GizmosExt
	{
		...
		public static void DrawCross(Vector3 atPos, float sideLength = 0.5f)
		{
			Gizmos.DrawLine(atPos - Vector3.right * sideLength, atPos + Vector3.right * sideLength);
			Gizmos.DrawLine(atPos - Vector3.up * sideLength, atPos + Vector3.up * sideLength);
			Gizmos.DrawLine(atPos - Vector3.forward * sideLength, atPos + Vector3.forward * sideLength);
		}
		...
		public static void DrawCenteredWireArc(Vector3 center, float radius, float angle, int segments = 20, Quaternion rotation = default(Quaternion))
		{
			if (rotation.Equals(default(Quaternion)))
				rotation = Quaternion.identity;
			rotation *= Quaternion.AngleAxis(-angle * 0.5f, rotation*Vector3.up);
			DrawWireArc(center, radius, angle, segments, rotation);
		}
